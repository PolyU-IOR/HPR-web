...
To boost performance on LP (and CCQP), HPR-LP uses two key enhancements: **restart strategies** and **adaptive updates of the penalty $\sigma$**. These are motivated by the $O(1/k)$ bounds in Theorem&nbsp;3. For completeness, Algorithm 2 shows the full HPR-LP framework.
...
2. **Case $\mathcal{T}_1 = \lambda_A I_m - A A^*$ with $\lambda_A \geq \|A\|_2^2$.**  
   This choice applies when $l_c \neq u_c$ or when solving the system in the above case is too expensive. The $y$-update takes the form
...
For general LP problems, set $\mathcal{T}_1=\lambda_A I_m- AA^*$ with $\lambda_A \ge \|A\|^2_2$ in HPR-LP. The update for $y^{r,t+1}$ is:
